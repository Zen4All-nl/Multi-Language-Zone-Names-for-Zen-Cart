...
// only show $current_categories_id
  ...
  /* BOF Zen4All Multi Language Zone Names 1 of 2 */
  function zen_get_zone_name($country_id, $zone_id, $default_zone, $lang_id = '') {
    global $db;
    $language_id = (empty($lang_id) ? (int)$_SESSION['languages_id'] : $lang_id);
    $zone = $db->Execute("SELECT zn.zone_name
                          FROM " . TABLE_ZONES . " z
                          LEFT JOIN " . TABLE_ZONES_NAME . " zn ON zn.zone_id = z.zone_id
                            AND zn.language_id = " . (int)$language_id . "
  ...
  /* EOF Zen4All Multi Language Zone Names 1 of 2 */
...
// return an array with country zones
  function zen_get_country_zones($country_id) {
    global $db;
    $zones_array = array();
    /* BOF Zen4All Multi Language Zone Names 2 of 2 */
    $zones = $db->Execute("SELECT z.zone_id, zn.zone_name
                           FROM " . TABLE_ZONES . " z
                           LEFT JOIN " . TABLE_ZONES_NAME . " zn ON zn.zone_id = z.zone_id
                             AND zn.language_id = " . (int)$_SESSION['languages_id'] . "
                           WHERE z.zone_country_id = " . (int)$country_id . "
                           ORDER BY zn.zone_name");
    /* EOF Zen4All Multi Language Zone Names 2 of 2 */
